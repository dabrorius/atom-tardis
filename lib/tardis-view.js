'use babel';

export default class TardisView {

  constructor(serializedState) {


    // Create root element
    this.element = document.createElement('div');
    this.element.classList.add('tardis');

    // Create message element
    const message = document.createElement('div');
    message.textContent = 'The Tardis package is Alive! It\'s ALIVE!';
    message.classList.add('message');
    this.element.appendChild(message);

    const count = document.createElement('div');
    count.textContent = '3'
    this.element.appendChild(count);

    const pastIndicator = document.createElement('div');
    pastIndicator.textContent = '-'
    this.element.appendChild(pastIndicator);
  }

  setPath(path) {
    this.element.children[0].textContent = path;
  }

  setCount(count) {
    this.element.children[1].textContent = count;
  }

  setPastIndicator(onRight) {
    this.element.children[2].textContent = onRight > 0 ? `>> ${onRight}` : "|";
  }

  // Returns an object that can be retrieved when package is activated
  serialize() {}

  // Tear down any state and detach
  destroy() {
    this.element.remove();
  }

  getElement() {
    return this.element;
  }

}
